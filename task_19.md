...
```text
head ──► [ dummy ] ──► [ Node:A ] ──► [ Node:B ] ──► [ Node:C ] ──► nullptr
                                                                         ↑
tail ────────────────────────────────────────────────────────────────────┘
```
...
Поток 2 (dequeue): забирает A → head→[A]→[B]  (dummy удалён/освобождён)
Поток 3 (enqueue): new Node() → аллокатор вернул ТОТ ЖЕ адрес что был у dummy!
                   enqueue E:  head→[A]→[B]→[E]  (новый dummy = старый адрес)

Поток 1 просыпается:
  CAS(head, dummy_addr, A) → УСПЕХ (адрес совпал!)
  head теперь указывает на A
  НО: "dummy" на самом деле Node(E) — данные испорчены!
```
...
| Hazard Pointers   | Средняя   | Средняя            | O(N·P) памяти, N потоков                    |
| Epoch-Based (RCU) | Высокая   | Высокая            | Узлы живут до конца эпохи                   |
| shared_ptr        | Низкая    | Низкая             | atomic<shared_ptr> — не настоящий lock-free |