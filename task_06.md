...
**Race Condition** (состояние гонки) — это высокоуровневая логическая ошибка в порядке выполнения операций.

#### Как соотносятся

Каждый **data race** — это **race condition**, потому что поведение зависит от гонки между несинхронизированными доступами. Но не каждый **race condition** — **data race**, потому что можно иметь логическую гонку даже при полностью корректной синхронизации.

Примеры

* **Data race:** два потока делают `x++` над обычным `int x` без `mutex` и без `atomic`; тут есть конфликтующие доступы к одной памяти без синхронизации.

* **Race condition без data race:** два потока корректно через `mutex` проверяют “кто первый занял ресурс”; памяти они не ломают, но итог зависит от того, кто успел раньше.
...
Так как существует много архитектур процессоров, и не у всех из них есть store buffer / invalidate queue, для решения проблем синхронизации договорились оперировать теоретическими
барьерами памяти в виде `X_Y`, где `X` и `Y` могут принимать одно из двух значений – либо `Load`, либо `Store`. 

Смысл у этого такой. Если поставить барьер типа `X_Y` в любое место кода, – это
означает, что все операции типа `X` до барьера выполнятся гарантированно до всех операций
типа `Y` после барьера. То есть не может быть ситуации, когда операции `X` спустятся вниз за
барьер `X_Y`, а операции `Y` поднимутся наверх.

### `Acquire/release`-семантика

![alt text](images/memory_bariers.png)

Пару барьеров `LoadLoad/LoadStore` назвали `acquire`. А `LoadStore/StoreStore` – `release`. А все вместе – `acquire/release`-семантика.
