...
* **SQ (Submission Queue)** — userspace кладёт сюда задания (SQE): «прочитай fd X», «запиши буфер Y»
* **CQ (Completion Queue)** — ядро кладёт сюда результаты (CQE): «операция завершена, результат N»
* Оба буфера расположены в одной физической памяти — нет копирования данных между userspace и kernel

### Lifecycle операции

```text
1. Userspace пишет SQE в SQ (без syscall — просто запись в память)

2. io_uring_enter() — один syscall для ВСЕЙ пачки операций
   (или вообще не нужен в SQPOLL режиме)

3. Ядро выполняет операции асинхронно

4. Ядро пишет CQE в CQ (без syscall для userspace)

5. Userspace читает CQ в цикле (без syscall — просто чтение памяти)
```
...
### SQPOLL режим: нулевые syscall

В режиме `IORING_SETUP_SQPOLL` ядро запускает специальный kernel-поток, который непрерывно опрашивает SQ:

```text
Userspace:  пишет SQE в SQ → сразу идёт дальше (0 syscall!)
Kernel:     SQPOLL-поток видит новые SQE → выполняет
            пишет CQE в CQ
Userspace:  читает CQE из CQ (0 syscall!)
```

Это максимальная производительность — весь I/O без единого syscall. Платой является то, что kernel-поток постоянно крутится на ядре CPU.
